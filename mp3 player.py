...
class PemutarMusik(Frame):
    ...
    def inisialisasiFile(self, file, path):
        ...
        if self.diStop:
            ...
            if self.b:
                if self.a.get() == "Play":
                    ...
                    self.a.set("Play")
                    self.pauseAutoStop = False
                    pygame.mixer.music.pause()
                    self.pauseAutoStop = True
            elif a != 0:
    ...
    def bukaFile(self):
        ...
        if bukaFile != "":
            for i in bukaFile:
                ...
                self.inisialisasiFile(nama, lokasi)
    ...
    def slider(self):
        w1 = Scale(from_=0.00, to=1.0, resolution=0.01, command=self.volume, orient=HORIZONTAL, length=300, label='Volume: ', showvalue=True)
        w1.pack()
        w1.set(0.50)
    ...
    def getPosisi(self):
        ...
        if posisi == -1 and self.berakhir and self.putarLagi:
            self.putarMusik()
        self.b = pygame.mixer.music.get_busy()
        if len(self.penampungFile) == 0 and not self.b and not self.diStop:
            self.nama.set("Pemutar Selesai")
            self.a.set("Play")
